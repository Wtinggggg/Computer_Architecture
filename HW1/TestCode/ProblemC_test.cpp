...
int main() {
    uint16_t h[] = {0x3C00, 0xC000, 0x7BFF, 0x0001, 0xFC00, 0x7C00, 0x0000, 0x3555, 0x3800, 0xB800, 
                    0x0400, 0x7E00, 0x3E00, 0x3F00, 0x4200, 0xC200, 0x47C0, 0x5000, 0xE000, 0x7800 };
    for ( int i = 0 ; i < 20 ; i++ )
        printf("0x%.4x >> 0x%.8x\n",h[i] , fp16_to_fp32(h[i]));
